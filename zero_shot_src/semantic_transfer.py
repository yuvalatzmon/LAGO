"""
A python module to support this zero-shot learning framework.
...
from utils_src.ml_utils import temporary_random_seed
...
class semantic_transfer_Layer(Layer):
    ...
    def _set_softgroups_init_by_ZS_type(self):
        ...
        def one_hot_groups_init_wrapper(shape):
            """
        ...
        def random_init(shape):
            with temporary_random_seed(common_args.SG_seed):
                M = np.random.rand(*shape)

            M = M - M.mean()
            return M

        def noisy_uniform_init(shape):
            """
...
def get_attributes_groups_ranges_ids(attributes_names):
    """
    ...
    if common_args.model_name=='LAGO':
        ...
        if common_args.model_variant == 'Singletons':
            # Singletons variant has one group per attribute
            return np.arange(1, attributes_dim)
        elif common_args.model_variant=='LAGO_KSoft':
